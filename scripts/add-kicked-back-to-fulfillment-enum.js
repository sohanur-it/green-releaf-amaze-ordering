...
/**
 * Add 'kicked_back_to_fulfillment' to modification_type enum
...
async function addEnumValue() {
    try {
        console.log('➡️  Adding "kicked_back_to_fulfillment" to modification_type enum...');
        ...
        await query(`
            ...
            EXCEPTION
                WHEN duplicate_object THEN 
                    RAISE NOTICE 'Enum value "kicked_back_to_fulfillment" already exists';
            END $$;
        `);
        
        console.log('✅ Enum value "kicked_back_to_fulfillment" added successfully');
    } catch (error) {
        // If the value already exists, that's fine
        if (error.message && error.message.includes('already exists')) {
            console.log('ℹ️  Enum value "kicked_back_to_fulfillment" already exists');
        } else {
